...
![fingerprint-tee]({{site.cdn_baseurl}}/assets/image/fingerprint-build-ta-01.webp){: .center-image }
...
In QSEE, normally there is specific location in the SDK file tree for storing the TA code. It is at path:
{% highlight ruby %}
...
I put the SDK into the below location:
{% highlight ruby %}
...
{% endhighlight %}
It uses the TA name that is defined at the configuration as the image name, which is `TA_Name.elf`. 
Such as `fpctzapp.elf`. The `TA_Name.syms.elf` is the image file that containing the symbols table which can be used for debugging purposes.
